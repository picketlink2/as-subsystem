 ...
 */
public class IdentityProviderSecurityDomainHandler implements OperationStepHandler {
 
    public static final IdentityProviderSecurityDomainHandler INSTANCE = new IdentityProviderSecurityDomainHandler();
 
    private IdentityProviderSecurityDomainHandler() {
    }
 
    @Override
    public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
        final String securityDomain = operation.require("value").asString();
        
        ModelNode node = context.readResourceForUpdate(PathAddress.EMPTY_ADDRESS).getModel();
        
        node.get(ModelElement.COMMON_SECURITY_DOMAIN.getName()).set(securityDomain);
